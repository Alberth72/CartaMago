...
| Client Cycle | Cloud rollout unblocked; public menu, WhatsApp link, admin login/menu, and `create-order` smoke verified in production | Netlify deploy `6a7fbf1709294fda6d2c916f`, public smoke no console/network errors, WhatsApp `wa.me` link OK, admin menu/product visible, smoke order created and deleted | Real product edit/image upload still pending | Run owner edit/upload test and confirm QR update on phone |

## Next Slice

```text
Run the real owner cycle: edit one existing product in `/admin`, upload a real image, verify the public QR menu updates on phone, test WhatsApp with the restaurant admin, and replace temporary prices with confirmed prices.
